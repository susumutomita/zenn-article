...
ここでいうハーネスとは、AIエージェントによる変更を実行前・実行後・コミット前・CIで検査する、実行可能な制約群です。ルールレジストリは、その制約を同じインターフェースで登録し、一括実行する仕組みです。この記事では、TenkaCloudのルールレジストリとhooksの設計、そしてハーネス自体も人が作る以上ドリフトする、という実例を書きます。
...
ルールの中身は、たとえば「Secrets Managerのimportを禁止する」というものです。TenkaCloudは常設運用コストを抑える方針があり、秘匿値はSecrets Managerではなく、SSM Parameter StoreのStandard tierのSecureStringへ置くと決めています。[Standard tierはParameter Store自体の追加料金がありません](https://docs.aws.amazon.com/ja_jp/systems-manager/latest/userguide/systems-manager-parameter-store.html)。ただし、SecureStringで利用するAWS KMSの料金は別です。これを各行の正規表現走査で機械的に検出します。
...
前節の`secrets-manager-forbidden`のコメントと合わせると、同じ失敗パターンが繰り返されています。「チェックするとドキュメントに書いてあるがルールがない」「hookはあるが呼び出し先がない」「コメントが現在の構成を説明していない」という状態です。ハーネスは一度組んで終わりではなく、リファクタのたびにメンテナンスしないと、チェックしているつもりで何もチェックしていない期間が生まれます。
...
ただし、ルール単体のテストが通っても、`.claude/settings.json`からそのルールやスクリプトへ到達できるとは限りません。今回壊れていたのはルールの判定ロジックではなく、設定と実装を結ぶ配線でした。次に必要なのは、次のようなハーネスの死活監視です。
