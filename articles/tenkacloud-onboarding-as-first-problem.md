...
主催者が用意した問題環境を、チームごとのAWSアカウントへワンクリックで構築できます（各チーム側のアカウント準備は初回のみ）。採点ダッシュボードや障害注入機能もプラットフォームとして持たせています。AWS GameDayやCTFを、自分たちで開催するためのプラットフォームと考えると近いです。参加者は障害対応や設定ミスの修正などの課題を解きます。遊び方は、リアルタイムで得点を競うBattleと、自分のペースで問題を進めるChallengeの2種類があります。
...
LPからこの1問目までの実際の流れは、動画でも見られます（[英語版はこちら](https://youtu.be/6qMzFcP5dgw)）。
