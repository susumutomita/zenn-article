...
```text
対象パス:   references/chapter8/src/blockchain.zig、p2p.zig、main.zig、docker-compose.yml
...
期待結果:   3ノードが高さ2・同一hash列へ収束し、重複と改ざんブロックを拒否する
```
...
services:
  node1:
    ...
    command: >-
      ...
      && (sleep 4; printf 'seed\n'; sleep 60)
      | /book-cache/$$NODE_ID/out/bin/chapter8 --listen 3000"
  ...
  node2:
    ...
    command: >-
      ...
      --global-cache-dir /book-cache/global
      --prefix /book-cache/$$NODE_ID/out
      && /book-cache/$$NODE_ID/out/bin/chapter8 --listen 3000 --connect node1:3000"
...
1. node1、node2を接続し、node1で `seed` ブロックを採掘する。
2. node3を後から起動し、`GET_CHAIN`で既存の `seed` を取得する。
3. node3をnode1とnode2の両方へ接続し、`gossip` ブロックを採掘する。
4. 3ノードへ `GET_CHAIN` を送り、高さ2かつ2つのhashが完全一致することを `cmp` で確認する。
5. 三角トポロジーを戻ってきたindex=2のブロックが、各ノードで1回だけ追加されたことを確認する。
6. 固定長バッファを超えるhexと小数のタイムスタンプを送り、node1が生存し、チェインも変わらないことを確認する。
7. `data`だけを書き換えたブロックを送り、node1のチェインが変わらず `invalid_pow` になることを確認する。
8. 成否にかかわらず `docker compose down --remove-orphans` を実行する。

成功時は、実行ごとに異なる2つのhashとともに次の形で終了します。
...
3つのターミナルでA、B、Cの順に起動します。Bを起動してAで `seed` を入力した後、Cを後発起動すると `GET_CHAIN`による追いつきを観察できます。Cで `gossip` を入力すると、B経由とAへの直接経路の両方が生まれます。
