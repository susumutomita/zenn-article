...
> 2. 次章以降で**複数ブロックの追加**、**PoW**、**P2Pネットワーク**を順次取り上げます。デジタル署名、残高モデル、フォーク選択は本書の実装範囲外です。
...
この段階では、まだハッシュ計算やマイニングは実装していません。
...
ブロックチェインの肝はハッシュの計算です。ブロックの`hash`フィールドは、ブロック内容全体(index, タイムスタンプ, prev_hash, dataなど)から計算されるハッシュ値です。Zigの標準ライブラリにはSHA-256などのハッシュ関数実装が含まれているので、それを利用してハッシュ計算をします。
...
1. **`std.mem.toBytes(value)`** や **`std.mem.asBytes(&value)`** を使う。
2. **`@bitCast()`** を使って独自に「生のバイト列」へ変換する処理を書く。

今回のコードでは **`@bitCast()`** を活用し、**`toBytes`** という小さな関数を定義しています。これは「**任意の型 `T` の値を、メモリ上の生のビット列として `[@sizeOf(T)]u8` の固定長配列に再解釈して返す**」処理です。呼び出し側で配列をローカル変数に保持し、そのアドレスを `hasher.update` へ渡します。
...
3. **固定長配列 `[@sizeOf(T)]u8` を値として返す**

   Zigの組み込み関数 `@bitCast(value)` は引数を1つ取り、代入先の型に基づいてメモリ内容を再解釈します。
   ここでは `value`(整数など)を「同じサイズのバイト配列」に再解釈しています。

4. **呼び出し側で配列を保持して `&bytes` を渡す**

   関数内のローカル配列へのスライスを返すと、その配列の寿命が終わった後を参照しかねません。本書では配列を値で返し、`index_bytes` などのローカル変数に保持してから `hasher.update(&index_bytes)` とします。
...
Zigの `std.ArrayList(T)` は使用前に `init(allocator)` で初期化します。また、使い終わったら `deinit()` を呼び出してメモリを解放します。
...
ブロックの`index`、`timestamp`、`prev_hash`、全トランザクションの`sender`、`receiver`、`amount`を順にハッシュへ投入します。
トランザクションをブロックに含めたことで、ハッシュ計算時に考慮すべきデータも増えます。`calculateHash`関数では、ブロック内の全トランザクションの内容もハッシュ入力に追加します。
...
> **発展:マークルツリー**
> トランザクション数が増えると、各トランザクションをすべて直接ハッシュ計算するのではなく、**マークルツリー**を使って1つのルートハッシュにまとめる手法が一般的です。ビットコインなどでは、このマークルルートだけをブロックヘッダーに入れ、ブロック全体を効率的に検証できる仕組みにしています。
> 本書では、すべてのトランザクションを順にハッシュする学習用の方式を採用します。マークルツリーは実装範囲外ですが、実用チェインとの差として覚えておきましょう。
