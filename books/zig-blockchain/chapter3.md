...
2. **バリデータ選出アルゴリズム**: ランダムな値を用いて、各バリデータのステーク量に比例して当選確率が割り当てられるように選出します。具体的には、全バリデータのステーク合計を`S`とし、`0`から`S-1`までの乱数`r`を一つ発生させます。あとはバリデータのリストを走査して累積和が`r`を超えた最初のバリデータを当選とします（ルーレット選択法）。
