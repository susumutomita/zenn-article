...
ZigはC言語に近い構文と性能を持ちながらも、メモリセーフティやエラーハンドリングなどの機能を備えています。本チュートリアルではその特性に触れながら、学習用にブロックチェインの基本構造を実装していきます。
...
コンテナ間通信とネットワーク: 同じdocker-compose.yml内で定義されたコンテナ同士は、[デフォルトで共通のネットワークに接続されます](https://docs.docker.jp/compose/networking.html#)。そのため、特別な設定をしなくても互いに通信可能で、各コンテナはサービス名をホスト名（DNS名）としてお互いを認識できます。つまり、Composeで定義したサービス`node1`からサービス`node2`に対しては、コンテナ内でホスト名`node2`を指定することで通信できます。
...
上記のYAMLでは、`main`ブランチへのプッシュやプルリクエストをトリガーとしてワークフローが走ります。ジョブはUbuntu環境で実行されます。コードのチェックアウトを終えたら次にコミュニティ提供の`goto-bus-stop/setup-zig`アクションを使ってZigコンパイラをインストールしています。`with:`セクションでZigのバージョンを指定して利用するZigのバージョンを固定しています。最後に`zig fmt --check .`でフォーマットを確認し、`zig build test`でプロジェクトのビルドおよびテスト（Zigのビルトインテストを利用）を行います。
