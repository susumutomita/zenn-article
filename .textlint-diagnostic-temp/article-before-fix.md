...
ここでいうハーネスとは、AIエージェントによる変更を実行前・実行後・コミット前・CIで検査する、実行可能な制約群です。ルールレジストリは、その制約を同じインターフェースで登録し、一括実行する仕組みです。この記事では、TenkaCloudのルールレジストリとhooksの設計を紹介します。加えて、ハーネス自体も人が作る以上ドリフトする、という実例を書きます。
